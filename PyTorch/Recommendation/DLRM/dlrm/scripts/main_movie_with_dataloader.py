...
flags.DEFINE_integer("num_numerical_features", 171,
...
def main(argv):
    ...
    data_train = MovieLensDataset(data_path='/workspace/dlrm/notebooks/data/ml-25m/train.csv', batch_size=FLAGS.batch_size)
    data_test = MovieLensDataset(data_path='/workspace/dlrm/notebooks/data/ml-25m/test.csv', batch_size=FLAGS.batch_size)

    def collate_fn(batch):
        batch = batch[0]
        return (batch[0].to(FLAGS.base_device),
                batch[1].to(FLAGS.base_device),
                batch[2].to(FLAGS.base_device))

    data_loader_train = DataLoader(data_train, collate_fn=collate_fn, shuffle=True)
    data_loader_test = DataLoader(data_test, collate_fn=collate_fn, shuffle=True)
